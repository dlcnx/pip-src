...
def pip(s):
    ...
    try:
        ...
        def w_or_a():
            ...
            if choose_wa == "1":
                with open("pip/pip.ini", "a") as f:
                    f.write(content_src)
            elif choose_wa == "2":
                with open("pip/pip.ini", "w") as f:
                    f.write(content_src)
            else:
                return w_or_a()

        # 若配置文件存在则让用户选择
        if os.path.exists("pip/pip.ini"):
            print("检测到配置文件已存在！\n[1]追加配置 [2]直接覆盖\n")
            w_or_a()
        else:
            with open("pip/pip.ini", "w") as f:
                f.write(content_src)
        # 创建结束
        print("\n成功创建文件！\n生成的pip文件夹放入 C:/Users/用户名 即可为指定用户pip换源\n")
    except OSError:
        print("\n文件修改失败，请检查pip文件夹异常!\n")
